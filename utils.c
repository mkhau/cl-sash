...
makeArgs(const char * cmd, int * retArgc, const char *** retArgv)
{
	...
	*retArgv = NULL;
	...
	// modification mangeuse de mémoire
	len += 1024;
...
//		printf("cp=%s, cpOut=%s, argument=%s\n", cp, cpOut, argument);
...
arithString(char* src)
{
	...
	if (res != NULL){
		...
		*res++;
		strcpy(buf2, res);
		op1 = arithString(buf1);
		op2 = arithString(buf2);		
		if (strcmp(op1, op2) == 0){
			res = "TRUE";
		} else {
			res = "FALSE";
	...
	if (res != NULL){
		...
		*res++;
		...
		op1 = arithString(buf1);
		op2 = arithString(buf2);		
		if (strcmp(op1, "Guillemet manquant") == 0 || strcmp(op2, "Guillemet manquant") == 0){
			res = "Guillemet manquant";
			return res;
		}
